...
- Bumped `vitest` and `@vitest/coverage-v8` from `^2.1.0` to `^4.1.5` to close Dependabot alert #1 (esbuild < 0.25.0 GHSA-67mh-4wv8-2f99, nested under vitest 2.x's pinned vite). `npm ls esbuild` now reports only `esbuild@0.25.12` (via `vite@6.4.2`). Skipped vitest 3.x as an intermediate step — Daniel's other forks are already on 4.1.5 so the breaking-change set is identical with one fewer migration. Prior triage attempt to use `"overrides": { "esbuild": "^0.25.0" }` (commit a8d22ca6) is documented to have broken vitest 2.x worker IPC and was not repeated. Vitest 4 enforces that `vi.fn().mockImplementation(() => ...)` factories must use the `function` keyword (not arrows) when consumed via `new` — five test files were updated accordingly: `tests/setup.ts` (ResizeObserver), `tests/electron/setup.ts` (BrowserWindow / Notification / Tray / MenuItem mocks), `tests/electron/notifications.test.ts` (four local Notification overrides), `tests/integration/fileOpening.test.tsx` (FileReader + PDFRenderer), `tests/unit/stores/documentStore.test.ts` (PDFRenderer). No production code touched. Suite returns `2517 passed / 9 skipped / 1 todo / 0 failed` (matches pre-bump baseline exactly).
