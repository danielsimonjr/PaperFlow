...
- Bumped `postcss` from 8.5.6 to 8.5.13 (GHSA-qx2v-qp2m-jg93 / CVE-2026-41305 — XSS via unescaped `</style>` in CSS stringify output). Dev-scope only; postcss is consumed during the Vite/Tailwind build pipeline. Verification: `npm run typecheck && npm run build && npx vitest run` all green (2517/9skip/1todo, baseline preserved).
- Bumped `uuid` from 11.1.0 to 14.0.0 (runtime dependency). Closes Dependabot alerts #83, #85 (GHSA-w5hq-g745-h8pq — out-of-bounds buffer write in `v3()`/`v5()`/`v6()` when the caller-supplied `buf` argument has an invalid `offset`). PaperFlow only uses `v4()` (UUIDs from `crypto.randomUUID`-class entropy, no buf argument) across `annotationStore`, `signatureStore`, `textStore`, and the four `lib/offline/*` modules — so the vulnerable code paths were never reachable from this codebase. The bump is still applied to clear the alert and to pull in v14's tightened bounds-check (`RangeError` thrown if `offset < 0 || offset + 16 > buf.length`). Breaking changes from 11→14 reviewed against PaperFlow's runtime: v12 dropped CommonJS (we are `"type": "module"`, OK); v12 raised TS minimum to 5.2 (we use 5.7, OK); v12 dropped node@16 (Electron 40 ships node@22, OK); v13 made browser exports default (we run in browser + Electron renderer, both browser-like contexts, OK); v14 requires global `crypto` and node@20+ (Electron 40 satisfies both). Verification: `npm run typecheck && npm run build && npx vitest run` all green (2517/9skip/1todo, baseline preserved across two consecutive runs to filter known integration-test flake).
- Bumped `tar` 7.5.2 → 7.5.13 and `picomatch` 4.0.3 → 4.0.4 in the three `tools/*` subproject lockfiles (`tools/create-dependency-graph`, `tools/compress-for-context`, `tools/chunking-for-files`). Closes Dependabot alerts 59-82 (12× tar advisories: GHSA-8qq5-rm4j-mr97, -r6q2-hw4h-h46w, -34x7-hfp2-rc4v, -83g3-92jg-28cx, -qffp-2rhf-9h96, -9ppj-qmqm-q256 covering CVE-2026-23745/-23950/-24842/-26960/-29786/-31802; 3× picomatch GHSA-3v7f-55p6-f55p / CVE-2026-33672). Both are transitive deps under `@yao-pkg/pkg@6.11.0`; the parent's existing semver ranges (`tar^7.4.3`, `picomatch^4.0.2`) already permit the fix versions, so the bump is a lockfile-only refresh with no parent-version change. Tools are dev-only `.exe` builders not bundled with the main app; main lockfile untouched.
