...
- **Unit & Integration Tests CI job restored to green** (`vitest.config.ts`, `electron/notifications.ts`, `tests/electron/notifications.test.ts`). Nine failures (`tests/integration/security/hardwareAuth.test.ts`, `tests/unit/lib/license/licenseStorage.test.ts`, `tests/unit/lib/offline/offlineStorage.test.ts`, `tests/integration/offline/offlineWorkflow.test.ts`) were caused by Vitest's default **forks** pool reusing a single child process across the ~150-file jsdom suite: jsdom's per-realm `ArrayBuffer`/typed arrays and `navigator` drift out of sync with Node's native Web Crypto realm, so at full-suite scale `SubtleCrypto.importKey` rejected jsdom-realm `BufferSource` ("2nd argument is not instance of ArrayBuffer … `ERR_INVALID_ARG_TYPE`") and `offlineStorage.getStorageStats` hit "navigator is not defined." Pinned `pool: 'threads'` so each worker's jsdom globals stay consistent with the crypto realm — no production code or security behaviour changed. The threads pool exposed a pre-existing latent flake in `notifications.test.ts` (leaked 500 ms grouped-notification `setTimeout`s firing after a test ended, against partial per-test Notification mocks → uncaught "notification.on/close is not a function"); fixed at the root: `NotificationManager.closeAll()` now also cancels pending group timers (a real behaviour bug — a deferred group could surface after callers asked to close everything), the two partial callback mocks now stub `close`, and both suites restore the complete default Notification mock and dispose their manager between tests. Full suite: 2517 passed, 9 skipped, 1 todo, 0 failed.
